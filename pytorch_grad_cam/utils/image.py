    ...
    for img in cam:
        ...
        if target_size is not None:
            if len(img.shape) > 2:
                ...
                target_length = target_size[0] if isinstance(target_size, (list, tuple)) else target_size
                img = zoom(np.float32(img), target_length / img.shape[0])
            else:
                raise ValueError("Invalid cam image shape")
...
def scale_accross_batch_and_channels(tensor, target_size):
    ...
    new_shape = (batch_size, channel_size) + result.shape[1:]
